...
subtitle: \small \vspace{-.1cm} A Reanalysis of the BA-Varenicline Trial Data
author: \vspace{-.3cm} \small  Thomas B. Arnold
#date: today
#date-format: long
format:
  pdf:
...
:::{.content-hidden}
*NOTE* **-- Overall**
- Major changes in this version are an increase in the number of bootstrap iterations (from B = 50 to B = 6,000) and a refocusing of the bootstrap process to estimate confidence intervals (CIs) only rather than estimating both CIs and coefficients.

- Notable that increasing the bootstrap iterations causes the interaction of BA and SHAPS score to lose significance in the moderator model.

- Another interesting consequence of the change is that our "enforce sparsity" procedure no longer functions in a useful way (the model either overfits when sparsity isn't enforced, or if even one single coefficient is zeroed out by sparsity enforcement the model fit is destroyed and the model has essentially no discriminatory power).

*TODO* **-- Overall**
[x] Previously coefficient estimates happened within the bootstrap, update to use single primary lasso for coefficient estimates and focus bootstrap on CI
[x] Check for all cascading consequences of this change above
[x] Fix variable names versus English names throughout
[x] Make sure figure numbers are correctly referenced
[x] Fix BA abbreviation to always use BA not BASC
[x] Ensure consistent case for "Lasso" throughout
[x] Fix code for using bootstrap only for CI
[x] Fix hard coded numeric values from model results within prose

***Reflection***
Why increase bootstrap iterations? Why handling the coefficient estimate outside of the bootstrap?

The main thing changed is two things: increased number of iterations for bootstrap loop (from 50 to 6000) and instead of estimating the coefficient inside the bootstrap loop, we estimate outside of the bootstrap loop and use the loop only for confidence interval estimation. The reason increase number of iterations is straightforward --- to improve the stability of estimates for confidence intervals.

One interesting thing we noticed as a result of increasing the bootstrap iterations --- in the interactions model, one interaction term previously deemed significant at lower number of bootstrap iterations is no longer significant when we increase iterations (i.e. the confidence interval now includes zero). What we learn is that in the event that we are forced to report results of bootstrap process in a low sample size environment with a low number of iterations, we need to be very cautious to always state our lack of confidence in the results. In low sample size situations, which is an environment where we often would use a bootstrap, the confidence interval estimates can be unreliable, and so reporting results from low iteration bootstrap processes can be dangerous. 

Why handle the coefficient estimate outside of the bootstrap loop? There is more statistical theory behind using the bootstrap for confidence interval estimation, but little on using the bootstrap to stabilize the central estimate itself. It is unclear what the consequence of estimating coefficients within the bootstrap loop has on the statistical properties of the subsequent coefficient estimates.

When we estimate the coefficient with the Lasso outside of the bootstrap loop, it's more straightforward to use the results of that non-bootstrap Lasso for variable selection because if you estimate coefficients inside of the bootstrap, some coefficients will, on average, be very small but non-zero because they will be zero in most bootstrap iterations, but non-zero in some others. That limits the effectiveness of one of the Lasso's most important tools, its use in variable selection and sparsification.
...
**Results:** The participant's Fagerstrom Test for Cigarette Dependence score and the participant's Nicotine Metabolite Ratio were all baseline predictors of abstinence controlling for treatment type. The models demonstrated fair to good discriminative ability. Our moderator analysis demonstrated that no variables could be identified as potential moderators of the therapy based treatment (BA or ST).
...
The `ftcd_score` is the participant's score on the Fagerstrom Test for Cigarette Dependence, a six-item survey intended to evaluate the quantity of cigarette consumption, dependence, and compulsion to smoke; answers are scored and summed to yield a total score of 0-10, with higher scores indicating a greater physical dependence on nicotine [@fagerstrom2011determinants]. The `bdi_score_pq1` is the participant's score on the Beck Depression Inventory, or BDI-II, a 21-item survey scored from 0 to 63 measuring the severity of depression, with higher scores indicating greater severity [@beck1996beck].

The `crv_total_pq1` score measures the participant's score on a questionnaire measuring preference for smoking over other traditionally rewarding activities, measured on a scale of 0 to 15, where 1 point is added for each time the participant chooses smoking [@spring2003reward]. Substitute and complimentary reinforcer scores (`hedonsum_n_pq1` and `hedonsum_y_pg1`) come from a participant's score of the cross-product of frequency (measured from 0 to 2) and level of enjoyableness (0 to 2) for a 45-item version of the Pleasant Events Schedule [@macphillamy1982pleasant]; based on the participant's report of whether an event is or is not associated with smoking, it is designated a complementary or substitute reinforcer, the cross products of which are summed.  

Anhedonia (`shaps_score_pq1`) is assessed with the 14-item Snaith-Hamilton Pleasure Scale (SHAPS), where 14 statements are ranked on a 4-point Likert scale, with higher scores indicating greater enjoyment of typically rewarding experiences [@snaith1995scale]. Nicotine metabolism ratio, otherwise known as nicotine metabolite ratio (`NMR`), is calculated as the ratio of 3'hydroxycotinine (3HC) to cotinine; NMR is a biomarker where higher NMR is associated with faster metabolism of nicotine [@siegel2020use]. 

:::{.content-hidden}
*TODO*
[x] After section above, replace variable names with their English counterparts (safe because we defined them clearly in paragraph above), OR if variable names need to be included for clarity, use the English name and then the variable name in a parenthetical.

***Reflection***
Better readability is important for communicating work.
:::
...
In addition to summary statistics available in Table 1 and the correlation matrix, we also inspect univariate histograms (omitted for space) for all variables. This reveals one non-normal variable, NMR, for which we consider transformations as noted below. Bivariate relationships are explored in Figure 1. Additionally we examine the distribution and association of all variables between levels of abstinence (`abst`), psychotherapy (`BA`), and pharmacotherapy (`Var`) (tables omitted for space). 

When dividing the sample for each variable by abstinence status (`abst`), we see that there is a significant difference between those who abstain and those who do not by pharmacotherapy treatment status (`Var`). This is because of the noted effectiveness of the pharmacotherapy treatment. We also observe marginal significance in the FTCD score variable (`ftcd_score`) with abstainers having lower baseline scores. This could be a confounder, but makes logical sense as a higher FTCD score indicates greater dependency which would make it more difficult to abstain. There are no other significant differences in variables between groups by abstinence status. Finally, when dividing the sample for each variable by psychotherapy (`BA`) we see that there are no significant differences in variables between groups by psychotherapy treatment status. Lastly, when dividing the sample for each variable by pharmacotherapy treatment status, we see one significant difference in abstinence status, essentially a mirror image of the difference discussed prior.


We note that certain survey variables, specifically FTCD score (`ftcd_score`), cigarette reward value (`crv_total_pq1`), readiness to quit (`readiness`), SHAPS score (`shaps_score_pq1`), complementary reinforcer score (`hedonsum_y_pq1`), and substitute reinforcer score (`hedonsum_n_pq1`) are ordinal but will be treated here as either numeric integers or continuous for the purpose of the modeling. This presents a limitation in that we are treating ordinal variables as evenly spaced. Doing so is more or less justifiable for different variables. For example, we are more justified in treating complementary and substitute reinforcer scores as continuous, given that each score is made up of the product of two ordinal scales; by using the cross-product, the study authors have made some assumptions about relative scale that obfuscates the ordinal nature of the original. 

Nevertheless, treating these variables as ordinal is untenable here, as it would make individual cell sizes incredibly small and lead to parameter proliferation in modeling. Another potential solution would be grouping ordinal variables into discernible levels, as with income, discussed below, but this would take an in-depth understanding of the meaning and spacing for each score that we do not have at present. Said another way, grouping the variables without fully understanding how relative distance between ordinal levels is expressed would lead to our making similar assumptions about the spacing between levels as we make in treating the variables as continuous. We note this is consistent with the handling of survey score variables in Table 1 of the original paper.
...
:::{.content-hidden}
*TODO*
[x] Add note about why we do not do polynomial sensitivity analysis

***Reflection***
A sensitivity analysis for removing polynomial terms would be a useful future endeavor, but we opt to keep the polynomial terms to allow us to interpret the non-linear relationship between some ordinal variables and the outcome. One weakness in using the polynomial terms as they currently exist in the model is that they are not orthogonalized polynomials (which were considered, but removed due to convergence issues). Said another way, even if the model preformed similarly without the polynomial, we would opt to include it to allow for interpretation of non-linear relationships between the ordinal variables and the outcome.
:::
...
# tbl <- tbl %>%
#   as_gt() %>% 
#   # cols_width( # Note: neither cols_width nore table.width work currently. cols_width
...
:::{.content-hidden}
*TODO*
[x] Replace old table 1 with table 1 similar to original paper

***Reflection***
The new table 1 is more informationally dense and succinctly conveys the different characteristics by treatment combination. It is particularly useful in supporting our decision to not use four separate treatment groups (2x2 factorial, as in the original paper) but rather include the pharmacotherapy and psychotherapy as independent variables because it illustrates the small cell sizes encountered as a result of the factorial handling.
:::
...
Other notable decisions include omission of the identification as Hispanic (`Hisp`) due to insufficient sample size (18 individuals), which is a limitation of our analysis. We considered taking the three race and ethnicity variables (`NHW`, `Black`, and `Hisp`) and producing a single race plus ethnicity variable with mutually exclusive groups for non-Hispanic white, non-Hispanic Black, Hispanic, and Other. However because there are only 18 individuals noted as identifying as Hispanic (and only two of those individuals identified as Black), this hypothetical combined race plus ethnicity variable would've suffered from the same cell size issue as other factors with many levels. 

We further note that based on our correlation matrix there is a very high correlation (well over 85%) between identification as non-Hispanic white and identification as Black. This issue combined with the issue noted previously lead us to also drop the non-Hispanic white variable. We also note that use of only menthol cigarettes is highly correlated with identification as non-Hispanic white, identification as Black, income, and education. This is a concern, but as menthol cigarette use may be a unique trait and we are conducting regularized regression, we elect to leave this variable in the model and allow the penalty to aid our choice. 

We conclude by grouping levels of education and income to limit parameter proliferation and allow for estimation and model convergence, given that Table 1 shows that there is insufficient sample within each level when not grouped. We also note that the continuous covariate NMR is right-skewed and could accommodate a log transformation to encourage normality. Log transforming skewed variables could help by encouraging convergence and simplifying the loss landscape. However, empirical results for model performance in models fit with and without the log transformation on NMR indicate that the model preforms similarly or better without the log transformation. Because interpreting untransformed coefficients is more straightforward than their transformed counterparts, and taking into account the lack of penalty in term of model fit for not transforming, we opt to leave NMR in the raw form. Finally, we note that there is a minor class imbalance issue (~2.1:10). The outcome is somewhat rare, and this could lead to model fitting issues. Correcting for this class imbalance is a potential area for improvement and a limitation.

:::{.content-hidden}
*TODO*
[x] Explain handling of NMR and reasons for decisions

***Reflection***
Despite observing that NMR is not normally distributed with a clear right skew, we opt to not apply a log transformation. We choose to not transform NMR because of empirical results (experimentation) that indicates model fit slightly improves when using raw NMR as opposed to the log of NMR (and we do not incur any penalty in terms of slower model fitting due to a more complex loss landscape). This has additional benefits, as the coefficient interpretation of untransformed variables is more simple than their transformed counterparts. The important lesson here is to not make assumptions about how transformation will affect model fit. One should always conduct experiments when making bifurcating choices in modeling pathways to support the decision made (so long as there is not a strong reason to make the decision in question that is unrelated to model fit, as with the polynomial contrasts). This is especially true when the model in question is not a simple linear model. This is in part because GLMs for binary outcomes assume that there is a linear relationship between the log odds of predictors and the outcome, but do not assume linearity between the predictors and the outcome itself. 
:::
...
We use a Lasso model to answer both questions at issue. To evaluate baseline variables that predict abstinence status, we construct a Lasso model that includes all main effects and polynomial terms for score variables. To evaluate baseline moderators of the effect of psychotherapy on abstinence status, we also use a Lasso model, but the model that we use this time includes all main effects and polynomial terms of scores, as well as interactions of those terms with psychotherapy (`BA`). We note that there is still the assumption that there is a linear relationship between the log odds of predictors and the outcome, but not an assumption of linearity between the predictors and the outcome itself. 
...
First, we perform an 80/20 validation training split, where each split has the same proportion of positive outcomes (where `abst = 1`). From this training data we generate five imputed datasets. These five datasets are stacked and used first to find the optimal $\lambda$ for our primary Lasso model via a process of cross-validation to minimize deviance (log-likelihood). Once this optimal $\lambda$ is determined, the primary Lasso model is fit on the five stacked imputed datasets. The primary Lasso model is so called because this model is responsible for estimating our coefficients (as to distinguish it from the Lasso models fit within the bootstrap procedure).

Having obtained out coefficient estimates, we proceed to the bootstrap procedure to estimate confidence intervals. First, we take a bootstrap re-sample of the raw, unimputed training data. We then produce five new imputed datasets and stack them. These stacked imputed datasets are used to fit a Lasso model from which coefficient values are extracted and stored to be used in the estimation of confidence intervals. This process is repeated 6,000 times to ensure reliable confidence interval estimates.

:::{.content-hidden}
*TODO*
[x] Update description of the model fitting procedure above to match new code

***Reflection***
N/A (Handled elsewhere, this change is the primary modification we make, see **overall** list at top of document)
:::
...
:::{.content-hidden}
*TODO*
[x] Update Baseline section to match new results and new workflow
...
```{r main effects lasso w bootstrap}
#GOOD; Q1; baseline
# Def All Params
...
```{r main effects coef lasso, eval = T}
...
```{r main effects bootstrap loop, eval = F}
...
```{r, fig.cap="Bootstrap Confidence Intervals for Lasso Coefficients", fig.height=5, fig.width=9, eval = T}
# plot the CIs
...
The polynomial variables are difficult to interpret in the same efficient manner as the continuous NMR or binary pharmacotherapy status, therefore we produce plots to enhance our understanding. The FTCD score demonstrated significant non-linear relationships with abstinence. As visualized in Figure 3, the relationship between FTCD score and abstinence probability follows a complex cubic pattern, with the highest probability of abstinence occurring at low levels of dependence and decreasing non-linearly as the score increases. This relationships between FTCD score and the outcome are produced while holding all other variables at zero (for numeric variables) or their reference level (for categorical variables). 

:::{.content-hidden}
*TODO*
[x] Add sentence explaining how plots are produced w/r/t other covariates

***Reflection***
Whenever interpretations and particularly predictive plots are produced, it is important to note precisely how these are produced (such as the values or levels of other covariates used when predicting on new data).
:::




```{r, fig.height=2.4, fig.width=6, fig.cap="Relationship between FTCD Score and Estimates"}
...
```{r, fig.cap="Relationship between Hedonsum Score and Estimates", fig.height=2.4, eval=F}
...
```{r, fig.cap="Lasso Calibration Plot: Validation Data"}
...
The model's discriminative ability was assessed using ROC curves and calibration plots (Figures 4 and 5). The Lasso model demonstrated good discriminative performance with an AUC of `r round(roc_valid$auc, 4)` on the validation set, though some overfitting is suggested by the higher training AUC of `r round(roc_train$auc, 4)`. The calibration plot reveals generally good agreement between predicted and observed probabilities at all ranges but shows increasing uncertainty at very high and very low probability ranges, as evidenced by the widening confidence bands around the Loess curve. The overall calibration suggests the model provides reasonably reliable probability estimates, especially in the more commonly observed lower probability ranges.
...
:::{.content-hidden}
*TODO*
[x] Update moderator section to match new results and new workflow

***Reflection***
N/A
:::

```{r interactions lasso w bootstrap}
#GOOD; Q2; Moderator 
# Def All Params
...
```{r interactions coef lasso, eval = T}
...
```{r interactions bootstrap loop, eval = F}
...
Results are presented for bootstrapped Lasso regression on the outcome abstinence, with main effects included and unpenalized. Interactions of baseline covariates with psychotherapy status (`BA`) are included and penalized, allowing the Lasso to select the interaction terms of consequence. The coefficients derived from the primary non-bootstrap Lasso fit are noted as such. Only coefficients with confidence intervals not containing zero are presented in the table of results (noted as significant). 
...
Our moderator analysis revealed that there were no significant interactions terms and therefore no baseline covariates which conclusively moderate the effect of psychotherapy treatment status on abstinence. The model also confirmed the robust effect of varenicline (OR = `r round(exp(coef_ci_sparse[["VarVarenicline","Primary_Estimate"]]),2)`, 95% CI (OR): `r round(exp(coef_ci_sparse[["VarVarenicline","Lower_2.5"]]),2)`-`r round(exp(coef_ci_sparse[["VarVarenicline","Upper_97.5"]]),2)`), though the wide confidence interval suggests considerable uncertainty in the effect size estimate.


```{r, fig.cap="Relationship between SHAPS Score and Estimates", eval = F}
...
:::{.content-hidden}
*TODO*
[x] Update conclusion to match new results
...
In this reanalysis of the BA-Varenicline trial data, we identified several important predictors of smoking cessation success among individuals with current or past MDD. Our primary analysis confirmed Varenicline's strong positive effect on abstinence and revealed complex non-linear relationships between cessation success and baseline characteristics such as nicotine dependence (FTCD score). Notably, higher nicotine metabolism rates were associated with improved cessation outcomes, a finding that warrants further investigation given its divergence from some previous research. 
