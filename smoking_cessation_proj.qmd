...
```{r main effects coef lasso, eval = F}
...
```{r, eval = F}
saveRDS(coef_matrix, file = "baseline_coef_CI-fix.rds")
saveRDS(nonzero_matrix, file = "baseline_nonzero_CI-fix.rds")

```

```{r, eval = T}
coef_matrix <- readRDS(file = "baseline_coef_CI-fix.rds")
nonzero_matrix <- readRDS(file = "baseline_nonzero_CI-fix.rds")
...
# first criteria
threshold <- .85 # make this dynamic later
...
#GOOD; Q2; Moderator (INTERACTION MODEL TO BE UPDATED)
# Def All Params
B <- 200      # num of bootstraps
m <- 5       # num of multiple imputations per bootstrap
k <- 5       # num of cv folds for cv.glmnet
...
```{r interactions coef lasso, eval = F}
...
```{r, eval = F}
saveRDS(coef_matrix, file = "moderators_coef_CI-fix.rds")
saveRDS(nonzero_matrix, file = "moderators_nonzero_CI-fix.rds")

```

```{r, eval = T}
coef_matrix <- readRDS(file = "moderators_coef_CI-fix.rds")
nonzero_matrix <- readRDS(file = "moderators_nonzero_CI-fix.rds")
