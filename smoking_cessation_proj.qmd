...
Results are presented for bootstrapped Lasso regression on the outcome abstinence, with main effects included and unpenalized. Interactions of baseline covariates with `BA` are included and unpenalized, allowing the Lasso to select the interaction terms of consequence. Only coefficients with confidence intervals not containing zero are presented in the table of results (noted as significant).
