...
[] Check for all cascading consequences of this change above
[] Fix variable names versus English names throughout
[] Fix BA abbreviation to always use BA not BASC
[] Ensure consistent case for "Lasso" throughout
[x] Fix code for using bootstrap only for CI
[] Fix hard coded numeric values from model results within prose
...
Results: The participant's Fagerstrom Test for Cigarette Dependence score, complementary reinforcers (i.e., pleasurable events associated with smoking), and the log of participant's Nicotine Metabolite Ratio were all baseline predictors of abstinence controlling for treatment type. The model demonstrated good discriminative ability (validation AUC=*TODO: XXX*). Our moderator analysis demonstrated that no variables could be identified as potential moderators of the therapy based treatment (BA or ST).
...
:::{.content-hidden}
*TODO*
[] After section above, replace variable names with their English counterparts (safe because we defined them clearly in paragraph above), OR if variable names need to be included for clarity, use the English name and then the variable name in a parenthetical.
...
:::{.content-hidden}
*TODO*
[] Update Baseline section to match new results and new workflow
...
```{r, eval = F}
saveRDS(coef_matrix, file = "baseline_coef_CI-fix_6k.rds")
saveRDS(nonzero_matrix, file = "baseline_nonzero_CI-fix_6k.rds")

```

```{r, eval = T}
coef_matrix <- readRDS(file = "baseline_coef_CI-fix_6k.rds")
nonzero_matrix <- readRDS(file = "baseline_nonzero_CI-fix_6k.rds")
...
As discussed in greater detail above, a higher score for complementary reinforcers denotes greater frequency of and/or more enjoyment of activities associated by the participant with smoking. It makes intuitive sense that, as our results suggest, participants whose more frequent and/or more enjoyable pleasurable events are with smoking would have a harder time quitting. For one thing, engaging in those frequent, pleasurable, and smoking-associated activities would likely lead them to wish to smoke.  Moreover, they might have fewer opportunities to engage in  pleasurable activities not associated with smoking, undermining their ability to find alternative sources of reward. These findings suggest that both nicotine dependence and the degree to which participants associate pleasurable activities with smoking have complex, rather than straightforward linear, relationships with abstention.


\btwocol

```{r, fig.height=2.4, fig.cap="Relationship between FTCD Score and Estimates"}
...
```{r, fig.cap="Relationship between Hedonsum Score and Estimates", fig.height=2.4, eval=T}
### Note: interesting dynamic where when B <- 1000 this is no longer significant,
### but when B <- 6000 it is. Volitile results.
...
The model's discriminative ability was assessed using ROC curves and calibration plots (Figures 4 and 5). The Lasso model demonstrated good discriminative performance with an AUC of `r round(roc_valid$auc, 4)` on the validation set, though some overfitting is suggested by the higher training AUC of `r round(roc_valid$auc, 4)`. The calibration plot reveals generally good agreement between predicted and observed probabilities at lower ranges, *TODO but shows increasing uncertainty at higher probability ranges, as evidenced by the widening confidence bands around the Loess curve. This pattern is not unexpected given the class imbalance in our data. The overall calibration suggests the model provides reasonably reliable probability estimates, especially in the more commonly observed lower probability ranges.*
...
:::{.content-hidden}
*TODO*
[] Update moderator section to match new results and new workflow
...
```{r, eval = F}
saveRDS(coef_matrix, file = "moderators_coef_CI-fix_6k.rds")
saveRDS(nonzero_matrix, file = "moderators_nonzero_CI-fix_6k.rds")

```

```{r, eval = T}
coef_matrix <- readRDS(file = "moderators_coef_CI-fix_6k.rds")
nonzero_matrix <- readRDS(file = "moderators_nonzero_CI-fix_6k.rds")
...
The diagnostic plots for the moderator analysis (Figures 7 and 8) suggest more modest model performance compared to our main effects model. The ROC curves show fair discriminative ability with an AUC of *TODO 0.59* on the validation set and *TODO 0.67* on the training set. This decreased performance relative to our previous model likely reflects the increased complexity introduced by interaction terms and the inherent difficulty in predicting treatment effect heterogeneity especially in small samples. We also note that the separation between ROC curves for the validation versus training set, where the training curve has a much higher AUC, is indicative of overfitting which is unsurprising given the large number of parameters and small amount of data. The calibration plot reveals deviations from the ideal diagonal, *TODO with the model showing a tendency toward over-optimistic predictions in the mid-range probabilities and under-prediction at higher probabilities.* *TODO The wide confidence bands around the Loess curve indicate considerable uncertainty in these predictions.* Despite these limitations, the diagnostic plots suggest the model provides fairly reliable probability estimates, but does tend to overfit the data. 
